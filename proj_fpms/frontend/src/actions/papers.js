...
export const getPapers = (id) => (dispatch, getState) => {
  axios
    .get(`/api/papers/${id}/get_user_posts`, tokenConfig(getState))
    .then((res) => {
      dispatch({
        type: GET_PAPERS,
        payload: res.data,
...
export const searchPapers = (value) => (dispatch, getState) => {
  axios
    .get(`/api/search/?search=${value}`, tokenConfig(getState))
    .then((res) => {
      dispatch({
        type: SEARCH_PAPERS,
        payload: res.data,
...
export const searchPapersAuthors = (value) => (dispatch, getState) => {
  axios
    .get(`/api/search/authors/?search=${value}`, tokenConfig(getState))
    .then((res) => {
      dispatch({
        type: SEARCH_PAPERS,
        payload: res.data,
...
export const searchPapersTitle = (value) => (dispatch, getState) => {
  axios
    .get(`/api/search/title/?search=${value}`, tokenConfig(getState))
    .then((res) => {
      dispatch({
        type: SEARCH_PAPERS,
        payload: res.data,
