...
export class PaperList extends Component {
  ...
  mlaNaming = (authors, author_name) => {
    //receive paper.authors
      ...
      if (temp.length <= 3) {
        if (temp.length === 2) {
          ...
          name =
            name +
            ", " +
            this.initials(temp[1], "apa") +
            ". " +
            " & " +
            this.initials(temp[2], "apa") +
            ". ";
        }
      } else {
        name = name + ", et al.";
  ...
  render() {
    ...
    if (group !== "") {
      displayPapers = displayPapers.filter((paper) => paper.group == group);
    }
    if (level !== "") {
      displayPapers = displayPapers.filter((paper) => paper.level == level);
    }

    if (impact_factor == "impact_factor" && group == "journal") {
      displayPapers = displayPapers
        .filter((paper) => paper.impact_factor_journal != null)
        .sort((a, b) =>
          a.impact_factor_journal < b.impact_factor_journal ? 1 : -1
        );
    } else if (impact_factor == "SJR" && group == "journal") {
      displayPapers = displayPapers
        .filter((paper) => paper.SJR_rating != null)
        .sort((a, b) => (a.SJR_rating < b.SJR_rating ? 1 : -1));
    } else if (impact_factor == "peer_reviewed") {
      displayPapers = displayPapers.filter(
        (paper) => paper.peer_reviewed != ""
      );
    }
    ...
    let UGCpapers = papers.map((paper) => {
      ...
      if (group !== "journal" && group !== "") {
        delete UGCpaper.Journal_Name;
      }
      if (group !== "conference_article" && group !== "") {
        delete UGCpaper.Conference_Name;
      }
      if (group == "report") {
        delete UGCpaper.Contribution_Status;
      }
    ...
    return (
      ...
      >
        ...
        <div className="text-right" style={{ textAlign: "right" }}>
          ...
          <button
            className="btn btn-success ms-auto d-print-none mx-2"
            onClick={(e) =>
              format == "UGC"
                ? this.exportToCSV(UGCpapers, this.props.user.username)
                : this.exportToCSV(papers, this.props.user.username)
            }
          >
            ...
            >
              <tbody>
                {displayPapers.map((paper) => (
                  <tr key={paper.id}>
                    <td>
                      {this.mlaNaming(paper.authors, author_name)}"{paper.title}
                      ".{" "}
